...
func NewRaftNode(id int, channel string, communicator transport.Communicator, peers []raft.Peer, join bool, getSnapshot func() ([]byte, error), proposeC <-chan string,
	...
	rc := &RaftNode{
		...
		unreachableLock:  sync.RWMutex{},
		unreachable:      make(map[uint64]struct{}, 0),
	}

	urls := []url.URL{}

	streamByType := transport.NewStreamsByType()
	rc.transport = &transport.Transport{
		Logger:             &zap.Logger{},
		DialTimeout:        10 * time.Second,
		DialRetryFrequency: 10,
...
func (rc *RaftNode) startRaft() {
	...
	if oldwal || rc.join {
		...
		rc.node = raft.StartNode(c, rc.rpeers)
	}
