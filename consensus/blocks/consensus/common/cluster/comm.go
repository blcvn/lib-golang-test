...
import (
	...
	orderer "github.com/blcvn/lib-golang-test/consensus/blocks/types/orderer"
...
func (c *Comm) DispatchSubmit(ctx context.Context, request *orderer.SubmitRequest) error {
	c.Logger.Warningf("Call H.requestContext")

	reqCtx, err := c.requestContext(ctx, request)
	if err != nil {
		return err
	}
	c.Logger.Warningf("Call H.OnSubmit")
...
func (c *Comm) requestContext(ctx context.Context, msg proto.Message) (*requestContext, error) {
	...
	c.Logger.Warnf("Channel %s", channel)
	// if channel == "" {
	...
	// cert := ExtractRawCertificateFromContext(ctx)
	// if len(cert) == 0 {
