...
func (n *node) start(fresh, join bool) {
	...
	n.logger.Debugf("etcdraft.node: Starting raft node: #peers: %v", len(raftPeers))
	...
	n.logger.Info("etcdraft.node: start n.run()")
	go n.run(campaign)
}

func (n *node) run(campaign bool) {
	n.logger.Infof("etcdraft.node: run .......")
