...
func (stream *Stream) Send(request *orderer.StepRequest) error {
	fmt.Printf("Stream.Send: stream.SendWithReport \n")
...
func (stream *Stream) SendWithReport(request *orderer.StepRequest, report func(error)) error {
	...
	if stream.Canceled() {
		fmt.Printf("Stream.SendWithReport: stream.Canceled \n")
	...
	if request.GetConsensusRequest() != nil {
		fmt.Printf("Stream.SendWithReport: request.GetConsensusRequest() \n")

		allowDrop = true
	}

	fmt.Printf("Stream.SendWithReport: call stream.sendOrDrop \n")
...
func (stream *Stream) sendOrDrop(request *orderer.StepRequest, allowDrop bool, report func(error)) error {
	fmt.Printf("Stream.sendOrDrop: allowDrop=%t \n", allowDrop)
	...
	case stream.sendBuff <- struct {
		...
		fmt.Printf("Stream.sendOrDrop: stream.sendBuff =%t \n", allowDrop)
...
func (stream *Stream) sendMessage(request *orderer.StepRequest, report func(error)) {
	...
	var err error
	defer func() {
		message := fmt.Sprintf("Send of %+v to %s(%s) took %v",
			request, stream.NodeName, stream.Endpoint, time.Since(start))
		if err != nil {
