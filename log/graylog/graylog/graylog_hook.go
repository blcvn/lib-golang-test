...
func (hook *GraylogHook) Fire(entry *logrus.Entry) error {
	...
	if hook.synchronous {
		...
		fmt.Println("Fire: add to queue #async")
		hook.wg.Add(1)
		hook.buf <- gEntry
	}
...
func (hook *GraylogHook) sendEntry(entry graylogEntry) {
	// fmt.Println("sendEntry: Send message to server ")
	...
	// fmt.Println("sendEntry: Start send: ", m)
